...
public class CutSimpleConvex : MonoBehaviour {
    ...
    public GameObject backup_base;
    public string CalledObject;
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if(other.gameObject.tag == "WoodBase")
        {
            Debug.Log("Fail");
        ...
        }else if (other.gameObject.tag == "Wood")
        {
            ...
            backup = other.gameObject;
            backup_base = other.transform.parent.gameObject;
            //CalledObject = other.transform.parent.gameObject.name;
            Debug.Log(CalledObject);
            backup_base.gameObject.GetComponent<Rigidbody>().isKinematic = true;
            Cut();
    ...
    void HandleBaryCentric(Vector3 newPoint, ref Vector2 newUV, ref Vector3 newNormal, Vector3[] points, Vector2[] uvs, Vector3[] normals)
    {
        ...
        float areaMainTri = Vector3.Cross(points[0] - points[1], points[0] - points[2]).magnitude;
        float a1 = Vector3.Cross(f2, f3).magnitude / areaMainTri;
        float a2 = Vector3.Cross(f3, f1).magnitude / areaMainTri;
        float a3 = Vector3.Cross(f1, f2).magnitude / areaMainTri;
